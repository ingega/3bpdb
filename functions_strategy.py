...
def establecerOrdenes(orden, ticker):  # order = 0 is for initial bet
    ...
    let's do this: the only reason that could be possible is that 
    ...
    3. close at market, and this is our new threshold, well we need to 
    update the adjust, and re-send set_orders 
    '''
    if ordensl == 0:  # let's go
        # call emergency_set, the control is order_id
        e = emergency_set(ticker)
        return e
    # now the tp order
    ordentp = mandarOrdenTP(ticker, cantidad, posicion, precioTP)
    msj = f"the TP order has set in {precioTP} with orderId {ordentp} "
    escribirlog(msj)
    # let's update the order
    order.update_order(orderSL=ordensl, orderTP=ordentp)
    # if necessary, cancel the open orders remain
    if orden > 1 and orden != 100:  # the 100 order os for emergency_set
    ...
    while True:
        ...
        if len(actual_orders) > 0:  # we have tickers to review
            while True:  # break gets when len(order)==0
                time.sleep(data.timeframe)
                # in some points, every order may be executed
                order = Order()
                actual_orders = order.read_order()
                ...
                for ticker in actual_orders:
                    ...
                    if r['epochIn'] > 0:
                        ...
                        if time_elapsed > (60 * data.barras):  # bars is in minutes
                            ...
                            escribirlog(msg)
                            profit = tie_exit(ticker)
                            # in tie, we need subtract adjust
                            adjust = r['adjust']
                            profit -= adjust
                            make_exit(ticker, profit, "tie")
                # after review all orders, we need to inform about the
                # function status, but also review if any ticker
                # get the 3bp
                review()
        else:
            break
